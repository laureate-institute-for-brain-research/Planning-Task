...
import csv, os, ctypes, ast, numpy, pyo, sys, logging #VMeter,
...
def get_one_rating(question, responses, g, clk):
    ...
    msg_continue= visual.TextStim(g.win,text="Press enter when you are done.",units='norm',pos=(0,0.7),color='red',height=0.075)
    msg = visual.TextStim(g.win,text="",units='norm',pos=(0,0.5),color=[-1,-1,-1],height=0.075,wrapWidth=int(1.75))
    msg.setText(question)
    scale_1 = visual.RatingScale(g.win, lineColor='Black', precision=1, low=1, high=7, mouseOnly = True, marker=visual.TextStim(g.win, text='l', units='norm', color='red'),
